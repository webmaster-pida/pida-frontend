...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // 1. Verificar documento principal del cliente (Webhook Custom)
    try {
        const userDoc = await db.collection('customers').doc(user.uid).get();
        if (userDoc.exists && userDoc.data().status === 'active') {
            console.log("Acceso autorizado por estado activo en documento principal.");
            return true;
        }
    } catch (e) {
        console.warn("Error verificando documento del cliente:", e);
    }

    // 2. Verificar Subcolección (Para compatibilidad con la extensión de Stripe)
    try {
        const subRef = db.collection('customers').doc(user.uid).collection('subscriptions');
    ...
    // 3. Verificar acceso VIP/Admin vía Backend
    try {
        const res = await fetch(`${PIDA_CONFIG.API_CHAT}/check-vip-access`, { method: 'POST', headers: headers });
        if (res.ok) { const r = await res.json(); if (r.is_vip_user) return true; }
    } catch (e) { }
    ...
    async function runApp(user) {
        ...
        // ============================================================
        // 0. DEFINICIONES DOM (Respetando tus IDs del index.html)
        // ============================================================
        const setupOverlay = document.getElementById('pida-setup-overlay');        // Modal del Robot (Ya existe en tu HTML)
        const subOverlay = document.getElementById('pida-subscription-overlay');   // Modal de Ventas (Ya existe en tu HTML)
        ...
        try {
            ...
            // ============================================================
            // 2. LIMPIEZA INICIAL (Reseteo visual)
            // ============================================================
            // Ocultamos AMBOS modales preventivamente usando tu clase CSS
            ...
            if (isOnboarding) {
                ...
                if (setupOverlay) setupOverlay.classList.remove('hidden'); // Quitamos la clase hidden al Robot
                // El subOverlay (ventas) ya lo ocultamos en el paso 2
            ...
            if (!hasAccess) {
                ...
                if (isOnboarding) {
                    ...
                    console.log("Onboarding activo: Esperando activación...");
                    // No hacemos nada visualmente, el setupOverlay ya está visible desde el paso 4
                    ...
                    if (subOverlay) subOverlay.classList.remove('hidden'); // Quitamos hidden a Ventas
                    if (setupOverlay) setupOverlay.classList.add('hidden'); // Aseguramos que Robot esté oculto
                }
                return; // 🛑 DETENEMOS TODO
            }

            // --- ACCESO CONCEDIDO ---
            // Aseguramos que TODO esté oculto
            if (subOverlay) subOverlay.classList.add('hidden');
            if (setupOverlay) setupOverlay.classList.add('hidden');
            ...
            async function setupPlanListener() {
                ...
                if (!badge) {
                    setTimeout(setupPlanListener, 500);
                    return;
                }

                // A. PINTADO INICIAL OPTIMISTA
                badge.classList.remove('vip-active', 'hidden');
                badge.removeAttribute('style');
                let initialPlan = userPlan || 'basico';
                let displayPlan = initialPlan.charAt(0).toUpperCase() + initialPlan.slice(1);
                if(displayPlan === 'Basico' || displayPlan === 'Basic') displayPlan = 'Básico';
                badge.innerHTML = `Plan <strong>${displayPlan}</strong>`;
                badge.classList.remove('hidden');

                // Si es la carga inicial y es básico, bloqueamos preventivamente
                if (initialPlan === 'basico' && btnPre && !btnPre.classList.contains('locked-feature')) {
                ...
                unsubscribePlanListener = db.collection('customers').doc(currentUser.uid)
                    .onSnapshot((doc) => {
                        let realPlan = 'basico';
                        let isTrial = false;
                        ...
                        console.error("Error escuchando cambios de plan:", error);
                    });
            }
